# DH.NCore AOT 兼容性分析报告

> 分析日期：2026-03-07  
> 分析版本：DH.NCore 11.13.x  
...
本报告对 `DH.NCore` 库进行全面的 AOT（Ahead-of-Time）兼容性扫描，识别所有在 NativeAOT 编译模式下会导致编译警告、运行时崩溃或功能失效的代码模式，并给出修复建议。

**结论：DH.NCore 当前大量依赖动态反射机制，其核心设计哲学（运行时动态类型解析、插件扫描、脚本引擎）与 AOT 的静态分析要求存在根本性冲突。** 若要支持 AOT，需要按模块分层处理：部分模块可通过添加 AOT 注解修复，部分模块需要提供替代 AOT 友好实现，少数模块必须标记为 `[RequiresDynamicCode]` 不支持 AOT。
...
**文件：** `DH.NCore/Reflection/ScriptEngine.cs`
...
该模块是 DH.NCore 最核心的基础设施，整个框架大量构建于此，AOT 问题影响范围极广。

### 2.1 动态类型查找 — `Type.GetType(string)`

**文件：** `DH.NCore/Reflection/Reflect.cs`
...
**文件：** `DH.NCore/Reflection/IReflect.cs`，`AssemblyX.cs`，`Reflect.cs`
...
**文件：** `DH.NCore/Reflection/IReflect.cs`，`DH.NCore/Model/ObjectContainer.cs`
...
**文件：** `DH.NCore/Reflection/DynamicXml.cs`，`DH.NCore/Reflection/DynamicInternal.cs`
...
**文件：** `DH.NCore/Serialization/SpanSerializer.cs`
...
**文件：** `DH.NCore/Serialization/Binary/BinaryUnknown.cs`，`JsonTest.cs`
...
**文件：** `DH.NCore/Xml/SerializableDictionary.cs`
...
**文件：** `DH.NCore/Serialization/Json/IJsonHost.cs`
...
**文件：** `DH.NCore/Serialization/Json/JsonReader.cs`，`JsonWriter.cs`
...
**文件：** `DH.NCore/Data/BinaryTree.cs`
...
**文件：** `DH.NCore/Caching/MemoryCache.cs`
...
**文件：** `DH.NCore/Web/PluginHelper.cs`
...
**文件：** `DH.NCore/Web/OAuthClient.cs`
...
**文件：** `DH.NCore/Http/IHttpHandler.cs`
...
**文件：** `DH.NCore/Http/ControllerHandler.cs`
...
**文件：** `DH.NCore/Extension/SpeakProvider.cs`，`Windows/SpeechRecognition.cs`
...
**文件：** `DH.NCore/Configuration/IConfigProvider.cs`
...
**文件：** `DH.NCore/Extension/EnumHelper.cs`
...
**文件：** `DH.NCore/Model/ObjectContainer.cs`
